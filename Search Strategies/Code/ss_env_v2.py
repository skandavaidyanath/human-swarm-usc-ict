# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 11:50:49 2019
...
class SearchStrategiesEnv(discrete.DiscreteEnv):
  ...
  def __init__(self, drone_entry_point=1, fire_direction='south', data_ready=False):
    ...
    if not data_ready:
      ...
      with open('ssenv_isd.pickle', 'wb') as fp:
        pickle.dump(initial_state_distrib, fp)
      with open('ssenv_P.pickle', 'wb') as fp:
        pickle.dump(P, fp)
        
      discrete.DiscreteEnv.__init__(self, num_states, num_actions, P, initial_state_distrib)
      
    else:
      initial_state_distrib = None
      P = None
      with open('ssenv_isd.pickle', 'rb') as fp:
        initial_state_distrib = pickle.load(fp)
      with open('ssenv_P.pickle', 'rb') as fp:
        P = pickle.load(fp)
    ...
    if action == 0:
      ...
      for transition in grid_status_updates:
        ...
        for new_rel_loc in possible_rel_locs:
          ...
          states.append(self.encode(new_state))
  ...
  def __burn_time_updates(self, update):
    '''
    ...
    for pos in possibilities:
      transition = {}
      new_grid = copy.deepcopy(grid)
      transition['prob'] = 1/len(possibilities) * update['prob']
  ...
  def __grid_status_updates(self, loc, grid, rel_loc):
    ...
    for update in search_updates:
      grid_status_updates.extend(self.__burn_time_updates(update))
