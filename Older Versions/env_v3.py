# -*- coding: utf-8 -*-
"""
Created on Fri Jun  7 09:15:21 2019
...
class HumanSwarmEnv(discrete.DiscreteEnv):
  ...
  def __init__(self, fire_update_probability = 0.0625, operator_play_probability=0.33, num_groups_to_save=3, warn_convince_prob = 0.5, spn_convince_prob = 0.75, on_convince_prob = 1, warning_end_prob = 0.9, spn_end_prob = 0.8, on_end_prob = 0.75, guide_end_prob = 0.2, death_factor = 0.05):
    ...
    self.on_convince_prob = on_convince_prob
                          ...
                          for e2n in range(2):
                            for e3w in range(3):
                              for e3n in range(2):
                                ...
                                state = self.encode([npf, fire_data, status_data, engagement_data])
                                if npf == s1 == s2 == s3 == e1w == e1n == e2w == e2n == e3w == e3n == 0:
  ...
  def encode(self, state):
    ...
    '''
    npf, fire_data, status_data, engagement_data = state
  ...
  def decode(self, i):
    ...
    out.reverse()
    npf = out[0]
    fire_data = [out[1], out[2], out[3], out[4]]
    status_data = [out[5], out[6], out[7]]
    engagement_data = [[out[8], out[9]], [out[10], out[11]], [out[12], out[13]]]
    return [npf, fire_data, status_data, engagement_data]
  

  
  def simulate(self, state, action):
    npf, fire_data, status_data, engagement_data = self.decode(state)
    ...
    if action == 0:
      ...
      rewards = []
      dones = []
      environment_reachable_transitions = self.simulate_environment(npf, fire_data, status_data, engagement_data)
      operator_reachable_transitions = self.simulate_operator(npf, fire_data, status_data, engagement_data)
      for i in range(len(environment_reachable_transitions)):
        ...
        for update in fire_updates:
          ...
          fire_changed_state[1] = update['fire_state']
      ...
      for update in fire_updates:
        ...
        fire_changed_state = self.encode([npf, update['fire_state'], status_data, engagement_data])
        new_states.append(fire_changed_state)
        rewards.append(0)
        dones.append(False)
    ...
    if action == 2:
      ...
      if status_data[0] != 5:
        ...
        for update in fire_updates:
          probs.append((self.fire_update_probability) * update['prob'])
          fire_changed_state = self.encode([npf, update['fire_state'], temp_status_data, temp_engagement_data])
          new_states.append(fire_changed_state)
          rewards.append(-10)
          dones.append(False)
        probs.append((1 - self.fire_update_probability))
        new_states.append(self.encode([npf, fire_data, temp_status_data, temp_engagement_data]))
        rewards.append(+10)
        dones.append(False)
      return probs, new_states, rewards, dones
  ...
  def simulate_operator(self, npf, fire_data, status_data, engagement_data):
    '''
    ...
    if not operator_busy:
      for drone in possible_drones:
        ...
        transition['state'] = self.encode([npf, fire_data, temp_status_data, temp_engagement_data])
        transition['reward'] = 0
  ...
  def simulate_environment(self, npf, fire_data, status_data, engagement_data):
    '''
    Simulates events that occur in the environment and modifies the states accordingly.
    Does not update the location of the fire. In any call to this function, only one group can be saved/ killed. 
    If a group is saved/ killed in a call to this function, nothing else happens.
    Possible updates are:
      1. Civilian group is saved (only from status 7)
      2. Civilian group has died (from any status except 7)
      3. Life has been found (only from status 0)
      4. Group has been convinced. Guide call available.  (from status 1,2,3)
      5. Warning has ended (only from status 1)
      6. SP Negotiation has ended (only from status 2)
      7. Operator negotiation has ended (only from status 3)
    '''
    ...
    if len(guiding_drones) > 0:
      for drone in guiding_drones:
        ...
        transition['state'] = self.encode([npf, fire_data, temp_status_data, engagement_data])
        transition['reward'] = +5000
        transition['done'] = False
        environment_reachable_transitions.append(transition)
      if len(guiding_drones) == 3:
        transition = {}
        transition['prob'] = 1 - self.guide_end_prob
        transition['state'] = self.encode([npf, fire_data, status_data, engagement_data])
        transition['reward'] = 0
        transition['done'] = False
        environment_reachable_transitions.append(transition)
      else:
        branch_point_one_transitions = self.branch_point_one(npf, fire_data, status_data, engagement_data)
        for transition in branch_point_one_transitions:
          transition['prob'] *= 1 - self.guide_end_prob
          environment_reachable_transitions.append(transition)
    else:
      environment_reachable_transitions.extend(self.branch_point_one(npf, fire_data, status_data, engagement_data))
    probs_sum = 0
  ...
  def branch_point_one(self, npf, fire_data, status_data, engagement_data):
    '''
    Utility function to make probability visualisation and calculation easier
    Called if at least one drone is not guiding
    '''
    branch_point_one_transitions = []
    
    kill_prob = (sum(fire_data)/4) * self.death_factor
    kill_possibilities = [i for i in range(3) if status_data[i] != 7]
    for drone in kill_possibilities:
      temp_status_data = copy.deepcopy(status_data)
      temp_status_data[drone] = 0
      transition = {}
      transition['prob'] = kill_prob * 1/len(kill_possibilities)
      transition['state'] = self.encode([npf, fire_data, temp_status_data, engagement_data])
      transition['reward'] = -500000
      transition['done'] = False
    ...
    if len(searching_drones) == 0:
      ...
      if len(warning_drones) > 0:
        branch_point_two_transitions = self.branch_point_two(npf, fire_data, status_data, engagement_data)
        for transition in branch_point_two_transitions:
            transition['prob'] *= 1 - kill_prob
            branch_point_one_transitions.append(transition)
      else:
        spn_drones = spn_drones = [i for i in range(3) if status_data[i] == 2]
        if len(spn_drones) > 0:
          branch_point_three_transitions = self.branch_point_three(npf, fire_data, status_data, engagement_data)
          ...
          if len(on_drones) > 0:
            branch_point_four_transitions = self.branch_point_four(npf, fire_data, status_data, engagement_data)
      ...
      for drone in searching_drones:
        ...
        if len(warning_drones) > 0:
          branch_point_two_transitions = self.branch_point_two(temp_npf, fire_data, temp_status_data, engagement_data)
          for transition in branch_point_two_transitions:
            transition['prob'] *= 1/len(searching_drones) * prob_lifefound * (1 - kill_prob)
            branch_point_one_transitions.append(transition)
          branch_point_two_transitions = self.branch_point_two(npf, fire_data, status_data, engagement_data)
          for transition in branch_point_two_transitions:
            transition['prob'] *= 1/len(searching_drones) * (1 - prob_lifefound) * (1 - kill_prob)
            branch_point_one_transitions.append(transition)
        else:
          if len(spn_drones) > 0:
            branch_point_three_transitions = self.branch_point_three(temp_npf, fire_data, temp_status_data, engagement_data)
            for transition in branch_point_three_transitions:
              transition['prob'] *= 1/len(searching_drones) * prob_lifefound * (1 - kill_prob)
              branch_point_one_transitions.append(transition)
            branch_point_three_transitions = self.branch_point_three(npf, fire_data, status_data, engagement_data)
            ...
            if len(on_drones) > 0:
              branch_point_four_transitions = self.branch_point_four(temp_npf, fire_data, temp_status_data, engagement_data)
            for transition in branch_point_four_transitions:
              transition['prob'] *= 1/len(searching_drones) * prob_lifefound * (1 - kill_prob)
              branch_point_one_transitions.append(transition)
            branch_point_four_transitions = self.branch_point_four(npf, fire_data, status_data, engagement_data)
            for transition in branch_point_four_transitions:
              ...
              for drone in searching_drones:
                ...
                temp_status_data[drone] = 6
                transition = {}
                transition['prob'] = (1 - kill_prob) * (prob_lifefound)
                transition['state'] = self.encode([temp_npf, fire_data, temp_status_data, engagement_data])
                transition['reward'] = 0
                transition['done'] = False
                branch_point_one_transitions.append(transition)
                transition = {}
                transition['prob'] = (1 - kill_prob) * (1 - prob_lifefound)
                transition['state'] = self.encode([npf, fire_data, status_data, engagement_data])
                transition['reward'] = 0
                transition['done'] = False
                branch_point_one_transitions.append(transition)
  ...
  def branch_point_two(self, npf, fire_data, status_data, engagement_data):
    '''
    Utility function to make probability visualisation and calculation easier
    ...
    for drone in warning_drones:
      if len(spn_drones) > 0:
        temp_status_data = copy.deepcopy(status_data)
        temp_status_data[drone] = 4
        branch_point_three_transitions = self.branch_point_three(npf, fire_data, temp_status_data, engagement_data)
        for transition in branch_point_three_transitions:
        ...
        if len(on_drones) > 0:
          temp_status_data = copy.deepcopy(status_data)
          temp_status_data[drone] = 4
          branch_point_four_transitions = self.branch_point_four(npf, fire_data, temp_status_data, engagement_data)
          for transition in branch_point_four_transitions:
            transition['prob'] *= 1/len(warning_drones) * self.warning_end_prob * (1 - self.warn_convince_prob)
            branch_point_two_transitions.append(transition)
          temp_status_data[drone] = 5
  ...
  def branch_point_three(self, npf, fire_data, status_data, engagement_data):
    '''
    Utility function to make probability visualisation and calculation easier
  ...
  def branch_point_four(self, npf, fire_data, status_data, engagement_data):
    '''
    Utility function to make probability visualisation and calculation easier
