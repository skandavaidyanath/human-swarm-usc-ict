# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 14:09:02 2019
...
class RSHumanSwarmEnv(discrete.DiscreteEnv):
  '''
  ...
  def __init__(self, group_type='stubborn_couple'):
        ...
        for dead_time in range(5):
          for guide_type in range(4):
            for neg_status in range(3):
              ...
              if status ==0 and dead_time >= 2 and guide_type == 0 and neg_status == 0:
    ...
    if action == 1:
      ...
      if status in [3,4]:
        for transition in time_updated_transitions:
          probs.append(transition['prob'])
          states.append(transition['state'])
          rewards.append(min(transition['reward'], -50))
          dones.append(transition['done'])
      else:
        for transition in time_updated_transitions:
          probs.append(transition['prob'])
          ...
          rewards.append(min(transition['reward'], -(op_busy * 100)))
          dones.append(transition['done'])
  ...
  def __time_updates(self, op_busy, status, dead_time, guide_type, neg_status):
    '''
    Updates the ending of a warning, an SPN, an ON, updates the dead_time an dop_busy time as well
    '''
    ...
    if status == 1:
      ...
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * self.dead_time_update_prob * (1 - self.warning_end_prob)
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * self.dead_time_update_prob * (1 - self.warning_end_prob)
      ...
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * (1 - self.dead_time_update_prob) * (1 - self.warning_end_prob)
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * (1 - self.dead_time_update_prob) * (1 - self.warning_end_prob)
      ...
      transition = {}
      transition['prob'] = self.dead_time_update_prob * self.operator_busy_time_update_prob * self.warning_end_prob * self.convince_probs['warn']
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * self.dead_time_update_prob * self.warning_end_prob * self.convince_probs['warn']
      ...
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * (1 - self.dead_time_update_prob) * self.warning_end_prob * self.convince_probs['warn']
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * (1 - self.dead_time_update_prob) * self.warning_end_prob * self.convince_probs['warn']
      ...
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * self.dead_time_update_prob * self.warning_end_prob * (1 - self.convince_probs['warn'])
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * self.dead_time_update_prob * self.warning_end_prob * (1 - self.convince_probs['warn'])
      ...
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * (1 - self.dead_time_update_prob) * self.warning_end_prob * (1 - self.convince_probs['warn'])
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * (1 - self.dead_time_update_prob) * self.warning_end_prob * (1 - self.convince_probs['warn'])
      ...
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * self.dead_time_update_prob * (1 - self.spn_end_prob)
      transition['state'] = self.encode([new_op_busy, status, new_dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = done
      if transition['prob'] > 0:
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * self.dead_time_update_prob * (1 - self.spn_end_prob)
      transition['state'] = self.encode([op_busy, status, new_dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = done
      if transition['prob'] > 0:
      ...
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * (1 - self.dead_time_update_prob) * (1 - self.spn_end_prob)
      transition['state'] = self.encode([new_op_busy, status, dead_time, guide_type, neg_status])
      transition['reward'] = 0
      transition['done'] = False
      if transition['prob'] > 0:
      ...
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * (1 - self.dead_time_update_prob) * (1 - self.spn_end_prob)
      transition['state'] = self.encode([op_busy, status, dead_time, guide_type, neg_status])
      transition['reward'] = 0
      transition['done'] = False
      if transition['prob'] > 0:
      ...
      # if group is convinced and both times are updated
      transition = {}
      transition['prob'] = self.dead_time_update_prob * self.operator_busy_time_update_prob * self.spn_end_prob * self.convince_probs['spn']
      transition['state'] = self.encode([new_op_busy, 4, new_dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = done
      if transition['prob'] > 0:
      ...
      # if group is convinced and only dead time is updated
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * self.dead_time_update_prob * self.spn_end_prob * self.convince_probs['spn']
      transition['state'] = self.encode([op_busy, 4, new_dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = done
      if transition['prob'] > 0:
      ...
      # if group is convinced and only op_busy time is updated
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * (1 - self.dead_time_update_prob) * self.spn_end_prob * self.convince_probs['spn']
      transition['state'] = self.encode([new_op_busy, 4, dead_time, guide_type, neg_status])
      transition['reward'] = 0
      transition['done'] = False
      if transition['prob'] > 0:
      ...
      # if group is convinced and neither times are updated
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * (1 - self.dead_time_update_prob) * self.spn_end_prob * self.convince_probs['spn']
      transition['state'] = self.encode([op_busy, 4, dead_time, guide_type, neg_status])
      transition['reward'] = 0
      transition['done'] = False
      if transition['prob'] > 0:
      ...
      # if group is not convinced and both times are updated
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * self.dead_time_update_prob * self.spn_end_prob * (1 - self.convince_probs['spn'])
      transition['state'] = self.encode([new_op_busy, 0, new_dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = done
      if transition['prob'] > 0:
      ...
      # if group is not convinced and only dead_time is updated
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * self.dead_time_update_prob * self.spn_end_prob * (1 - self.convince_probs['spn'])
      transition['state'] = self.encode([op_busy, 0, new_dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = done
      if transition['prob'] > 0:
      ...
      # if group is not convinced and only op_busy is updated
      transition = {}
      transition['prob'] = self.operator_busy_time_update_prob * (1 - self.dead_time_update_prob) * self.spn_end_prob * (1 - self.convince_probs['spn'])
      transition['state'] = self.encode([new_op_busy, 0, dead_time, guide_type, neg_status])
      transition['reward'] = 0
      transition['done'] = False
      if transition['prob'] > 0:
      ...
      # if group is not convinced and neither times are updated
      transition = {}
      transition['prob'] = (1 - self.operator_busy_time_update_prob) * (1 - self.dead_time_update_prob) * self.spn_end_prob * (1 - self.convince_probs['spn'])
      transition['state'] = self.encode([op_busy, 0, dead_time, guide_type, neg_status])
      transition['reward'] = 0
      transition['done'] = False
      if transition['prob'] > 0:
      ...
      transition = {}
      transition['prob'] = self.dead_time_update_prob * (1 - self.on_end_prob)
      transition['state'] = self.encode([op_busy, status, new_dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = done
      if transition['prob'] > 0:
      ...
      transition = {}
      transition['prob'] = (1 - self.dead_time_update_prob) * (1 - self.on_end_prob)
      transition['state'] = self.encode([op_busy, status, dead_time, guide_type, neg_status])
      transition['reward'] = reward
      transition['done'] = False
      if transition['prob'] > 0:
      ...
      transition = {}
      transition['prob'] = self.dead_time_update_prob * self.on_end_prob * self.convince_probs['on']
      ...
      transition = {}
      transition['prob'] = (1 - self.dead_time_update_prob) * self.on_end_prob * self.convince_probs['on']
      ...
      transition = {}
      transition['prob'] = self.dead_time_update_prob * self.on_end_prob * (1 - self.convince_probs['on'])
      ...
      transition = {}
      transition['prob'] = (1 - self.dead_time_update_prob) * self.on_end_prob * (1 - self.convince_probs['on'])
