# -*- coding: utf-8 -*-
"""
Created on Mon Jun 10 16:18:18 2019
...
class HumanSwarmEnv(discrete.DiscreteEnv):
  ...
  def __init__(self, fire_update_probability=0.0625, operator_play_probability=0.3, num_groups_to_save=3, warn_convince_prob=0.5, spn_convince_prob=0.75, on_convince_prob=1, warning_end_prob=0.9, spn_end_prob=0.8, on_end_prob=0.75, guide_end_prob=0.2, death_factor=0.05, data_ready=False):
  ...
  def __simulate_environment(self, npf, nps, fire_data, status_data, engagement_data):
    ...
    if len(guiding_drones) > 0:
      for drone in guiding_drones:
        ...
        transition['state'] = self.encode([npf, nps + 1, fire_data, temp_status_data, engagement_data])
        transition['reward'] = +5000
        transition['done'] = False
        environment_reachable_transitions.append(transition)
      num_guiding_drones = len(guiding_drones)  # not killable
      num_killable_groups = self.num_groups_to_save - nps - num_guiding_drones
      if num_killable_groups == 0:
        transition = {}
      ...
      num_killable_groups = self.num_groups_to_save - nps  # there are no guiding drones
      if num_killable_groups > 0:
  ...
  def __branch_point_one(self, npf, nps, fire_data, status_data, engagement_data):
    ...
    Called if at least one group is killable
    '''
    ...
    for drone in killable_drones:
      ...
      temp_status_data[drone] = 0
      transition = {}
      transition['prob'] = (1/num_killable_groups) * kill_prob
    ...
    transition = {}
    transition['prob'] = (1 - len(killable_drones)/num_killable_groups) * kill_prob
    transition['state'] = self.encode([npf + 1, nps, fire_data, status_data, engagement_data])   
    transition['reward'] = -500000
    transition['done'] = False
    branch_point_one_transitions.append(transition)
        ...
        if len(warning_drones) > 0:
          ...
          if len(spn_drones) > 0:
            ...
            if len(on_drones) > 0:
              ...
              temp_status_data[drone] = 6
              transition = {}
              transition['prob'] = 1/len(searching_drones) * (1 - kill_prob) * (prob_lifefound)
              transition['state'] = self.encode([npf + 1, nps, fire_data, temp_status_data, engagement_data])
              transition['reward'] = 0
              transition['done'] = False
              branch_point_one_transitions.append(transition)
              transition = {}
              transition['prob'] = 1/len(searching_drones) * (1 - kill_prob) * (1 - prob_lifefound)
              transition['state'] = self.encode([npf, nps, fire_data, status_data, engagement_data])
              transition['reward'] = 0
              transition['done'] = False
              branch_point_one_transitions.append(transition)
